...
class RacePlotterOneCar:
    def __init__(self, car, track, drs, safety_car=True, pitstops=True, weather=True, save=False):
        ...
        self.ax_time = self.figure.add_axes(
            [0.55, 0.85, 0.4, 0.1], frame_on=False)
        self.ax_speed = self.figure.add_axes([0.55, 0.58, 0.4, 0.25])
        self.ax_actions = self.figure.add_axes(
            [0.55, 0.25, 0.4, 0.3], frame_on=False)
        self.ax_race = self.figure.add_axes(
            [0.55, 0.14, 0.4, 0.07], frame_on=False)
        self.ax_results = self.figure.add_axes(
            [0.55, 0.05, 0.4, 0.07], frame_on=False)

        # Plot track
        track.plot_track(self.ax_track, show_drs=drs)
        self.mask = np.zeros((track.num_rows, track.num_cols, 4))
        # grey for unexplored areas
        self.mask[:, :, :3] = 0.7
        # fully masked to start with
        self.mask[:, :, 3] = 1
        self.mask_image = self.ax_track.imshow(self.mask, zorder=3)

        # Plot position
        position, heading = track.get_start_position_and_heading()
        self.car_marker = self.ax_track.plot(
            position.column, position.row, self.get_marker_from_heading(heading))[0]

        # Plot move & time
        self.move_text = self.ax_time.text(
            0, 1, 'Move #: 0', fontsize=20, va='top')
        self.time_text = self.ax_time.text(
            1, 1, 'Time: 0.0', fontsize=20, ha='right', va='top')
        ...
        trans = transforms.blended_transform_factory(
            self.ax_speed.transData, self.ax_speed.transAxes)
        self.safety_car_rect = Rectangle(
            [np.nan, 0], 0, 1, transform=trans, color='y', alpha=0.1)
        ...
        if drs:
            self.actions_text[Action.OpenDRS] = self.ax_actions.text(
                0, 0.5, 'DRS Active', va='center', fontsize=fs)
        if pitstops:
            self.actions_text[Action.ChangeTyres] = self.ax_actions.text(0, 0, 'Pit Stops: #0', va='bottom',
                                                                         fontsize=fs)
        # Add KERS indicator (similar to DRS)
        if hasattr(car, 'kers'):
            self.actions_text['KERS'] = self.ax_actions.text(
                0, 0.25, 'KERS', 
                va='center',
                fontsize=fs,
            ...
            # Add KERS energy rectangle in speed plot
            self.kers_start = None
            self.kers_rect = self.ax_speed.axvspan(
                0, 0,
                color='green',
                alpha=0.2,
                label='KERS Active'
        ...
        if safety_car:
            self.race_state_text['safety_car'] = self.ax_race.text(0, 0.5, 'Safety Car', va='center', ha='left',
                                                                   fontsize=fs)
        if weather:
            self.race_state_text['rain'] = self.ax_race.text(
                1, 0.5, f'Rain: 0', va='center', ha='right', fontsize=fs)
        
        # Add KERS indicator
        self.kers_text = self.ax_race.text(0.5, 0.5, 'KERS: 0%', 
                                         transform=self.ax_race.transAxes,
                                         bbox=dict(facecolor='white', alpha=0.7))
        
        # Add KERS bar in speed plot
        self.kers_rect = None
        if hasattr(car.dynamics_model, 'kers_energy'):
            self.kers_rect = self.ax_speed.axvspan(0, 0,
    ...
    def update(self, position, heading, action: Action, result: ActionResult, move_number: int, race_time: float,
        ...
        track_left, track_right, track_up, track_down = heading.rotate_from_car_to_track(
            track_state)
        self.mask[position.row, position.column:position.column +
                  track_right + 2, 3] = 0
        self.mask[position.row, position.column:position.column -
                  track_left - 2:-1, 3] = 0
        self.mask[position.row:position.row +
                  track_down + 2, position.column, 3] = 0
        self.mask[position.row:position.row -
                  track_up - 2:-1, position.column, 3] = 0
        self.mask_image.set_data(self.mask)

        # Update position: force x and y into 1D NumPy arrays
        try:
            x_val = np.atleast_1d(position.column)
            y_val = np.atleast_1d(position.row)
        except AttributeError:
            x_val = np.atleast_1d(position[0])
            y_val = np.atleast_1d(position[1])

        # Update position
        self.car_marker.set_data(x_val, y_val)
        ...
        # Update speed trace with KERS coloring
        horizon = 20
        self.speeds.append(self.car.speed)
        x = np.arange(max(0, len(self.speeds) - horizon), len(self.speeds))
        y = self.speeds[-horizon:]
        
        # Color the speed trace based on KERS activation
        if hasattr(self.car.dynamics_model, 'kers_energy'):
            self.speed_trace.set_color('green' if self.car.dynamics_model.kers_active else 'blue')
        else:
            self.speed_trace.set_color('blue')  # Default color
        ...
        if safety_car_active and self.safety_car:
            if self.safety_car_start is None:
                ...
                self.safety_car_rect.set_width(
                    move_number - self.safety_car_start)
        ...
        if Action.OpenDRS in self.actions_text:
            self.actions_text[Action.OpenDRS].set_backgroundcolor(
                'orange' if self.car.drs_active else 'none')
        if Action.ChangeTyres == action:
            self.num_pit_stops += 1
        if self.pitstops and Action.ChangeTyres in self.actions_text:
            self.actions_text[Action.ChangeTyres].set_text(
                f'Pit Stops: #{self.num_pit_stops}')

        # Highlight race state
        if self.safety_car and 'safety_car' in self.race_state_text:
            self.race_state_text['safety_car'].set_backgroundcolor(
                'orange' if safety_car_active else 'none')
        if self.weather and 'rain' in self.race_state_text:
            self.race_state_text['rain'].set_backgroundcolor('lightblue' if weather_state.rain_intensity > 0 else
                                                             'none')
            self.race_state_text['rain'].set_text(
                f'Rain: {weather_state.rain_intensity: .1f}')
        ...
        self.result_text['spun'].set_backgroundcolor(
            'orange' if result.spun else 'none')
        self.result_text['crashed'].set_backgroundcolor(
            'orange' if result.crashed else 'none')
        self.result_text['finished'].set_backgroundcolor(
            'orange' if result.finished else 'none')

        # Crashed figure colour
        self.figure.set_facecolor('r' if result.crashed else 'w')

        # Update KERS visualization
        if hasattr(self.car.dynamics_model, 'kers_energy'):
            kers_percent = (self.car.dynamics_model.kers_energy / 
                          self.car.dynamics_model.max_kers_energy) * 100
            
            # Update KERS text
            self.kers_text.set_text(f'KERS: {kers_percent:.0f}%')
            self.kers_text.set_bbox(dict(
                facecolor='lightgreen' if self.car.dynamics_model.kers_active else 'white',
                alpha=0.7
            ))
            
            # Update KERS activation zone in speed plot
            if self.car.dynamics_model.kers_active:
                if self.kers_rect is not None:
                    self.kers_rect.set_xy((move_number, 0))
                    self.kers_rect.set_width(1)
            
            # Color speed trace based on KERS activation
            if self.car.dynamics_model.kers_active:
                self.speed_trace.set_color('green')
            else:
                self.speed_trace.set_color('blue')
...
def save_season_test_results(filename: str, test_name: str, race_times_array: np.ndarray, finished_array: np.ndarray):
    ...
    # Add new results
    results[test_name] = {
        'race_times': race_times_array.tolist(), 'finished': finished_array.tolist()}
...
def plot_season_test_results(filename: str):
    ...
    race_times = [np.array(test_result['race_times'])
...
def plot_q_table(driver):
    ...
    for state, value_dict in driver.q_table.items():
        ...
        action = next(act for act, val in value_dict.items()
                      if val == max_value)
        ax.text(state[0], state[1], f'{max_value:.0f}',
                ha='center', va='center', c=colours[action], fontsize=8)
    ...
    for action, colour in colours.items():
        key_ax.text(x, 0, action if isinstance(action, str)
                    else action.name, color=colour, fontsize=12)
        x += 1
...
def plot_car_dynamics(car):
    ...
    speed_out_full_throttle = np.array(
        [dynamics.full_throttle(s) for s in speed_in])
    speed_out_light_throttle = np.array(
        [dynamics.light_throttle(s) for s in speed_in])
    speed_out_heavy_braking = np.array(
        [dynamics.heavy_brake(s) for s in speed_in])
    speed_out_light_braking = np.array(
        [dynamics.light_brake(s) for s in speed_in])
    ...
    ax_thr.legend([ft_lines[0], lt_lines[0]], [
    ...
    ax_brake.legend([hb_lines[0], lb_lines[0]], [
                    'Heavy braking', 'Light braking'])
...
def plot_multiple_championship_results(championship_results: dict):
    ...
    colour_array = np.tile(np.reshape(pos_colours, (num_drivers, 1, 4)), [
                           1, num_drivers, 1])  # (num_drivers, num_drivers, 4)
    ...
    for i, positions in enumerate(championship_results.values()):
        ...
        for j in range(num_drivers):        # loop over championship ranks
            ...
            # set transparency according to normalised count
            colour_array[j, i, -1] = scores[j, i]
    ...
    ax.set_title(
        'Number of times a driver finished in each position', fontsize=12)

    # Add text labels
    for row in range(num_drivers):
        for col in range(num_drivers):
            ax.text(
                col, row, f'{scores[row, col]*100: .0f}%', va='center', ha='center')
...
def plot_grip_sweep():
    ...
    for ig, grip in enumerate(grips):
        ...
        speeds[-1] = car.dynamics_model.max_cornering_speed(
...
def plot_tyre_degradation(n_samples=1):
    ...
    fig = plt.figure(figsize=(9, 5))
    ax = fig.gca()
    colours = {TyreChoice.Soft: 'orange',
               TyreChoice.Medium: 'purple', TyreChoice.Hard: 'b'}
    lines = {}

    for _ in range(n_samples):
        tyre_model = TyreModel()
        for choice in TyreChoice.get_choices():
            # loads up a new degradation curve
            tyre_model.new_tyres_please(choice)
            lines[choice] = ax.plot(
                age, tyre_model.get_grip(age), c=colours[choice])[0]
...
# def plot_race_stats(driver, track_layout, positions, speeds, steering_angles, race_time):
#     fig = plt.figure(figsize=(15, 10))
#     gs = gridspec.GridSpec(3, 2)

#     # Track visualization 
#     ax1 = plt.subplot(gs[0, 0])
#     plot_track(track_layout)
#     plot_path(positions)
#     ax1.set_title('Race Path')

#     # Speed plot with KERS overlay
#     ax2 = plt.subplot(gs[0, 1])
#     time_steps = np.arange(len(speeds))
#     ax2.plot(time_steps, speeds, label='Speed')
    
#     if hasattr(driver.car, 'kers'):
#         kers_data = driver.car.kers.history
#         kers_active = [h['active'] for h in kers_data]
#         kers_energy = [h['energy'] for h in kers_data]
        
#         # Add KERS activation zones
#         ax2_twin = ax2.twinx()
#         ax2_twin.fill_between(time_steps, 0, 100, 
#                             where=kers_active,
#                             color='green', alpha=0.2, 
#                             label='KERS Active')
#         ax2_twin.plot(time_steps, kers_energy, 
#                      color='green', linestyle='--', 
#                      label='KERS Energy')
#         ax2_twin.set_ylabel('KERS Energy')
...
#     # Add KERS statistics to race summary
#     stats_text = f"""Race Statistics:
#     Total Time: {race_time:.2f}s
#     Avg Speed: {np.mean(speeds):.2f} km/h
#     Max Speed: {np.max(speeds):.2f} km/h"""
    
#     if hasattr(driver.car, 'kers'):
#         kers_usage = sum(kers_active)/len(kers_active) * 100
#         total_energy = sum(h['energy'] for h in kers_data)
#         stats_text += f"\nKERS Usage: {kers_usage:.1f}%\nEnergy Recovered: {total_energy:.1f} units"
