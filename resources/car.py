...
class CarDynamicsModel1(CarDynamicsModel):
    def __init__(self, top_speed=300, tyre_multiplier=1.0, downforce_multiplier=1.0):
        ...
        # Add KERS system initialization
        self.kers_energy = 0
        self.max_kers_energy = 50
        self.kers_recovery_rate = 0.3 # 30% energy recovery from braking
        self.kers_boost_multiplier = 1.15  # Similar to DRS boost
        self.kers_active = False
        
        # Add path analysis attributes
        self.path_memory = []
        self.next_turn_distance = float('inf')

        self.kers_params = {
            'min_straight_length': 30,
            'min_distance_to_turn': 10,
            'min_speed': 30,
            'max_speed': 250
        }
        
        self.kers_performance_history = []
        self.learning_rate = 0.1
        self.performance_window = 50  # Number of activations to consider


    def update_kers_parameters(self, success_metric):
        """Update KERS parameters based on performance"""
        self.kers_performance_history.append(success_metric)
        
        if len(self.kers_performance_history) >= self.performance_window:
            recent_performance = np.mean(self.kers_performance_history[-self.performance_window:])
            
            # Adjust straight length requirement
            if recent_performance > 0.7:  # If successful more than 70% of the time
                self.kers_params['min_straight_length'] = max(20, 
                    self.kers_params['min_straight_length'] - self.learning_rate * 5)
            else:
                self.kers_params['min_straight_length'] += self.learning_rate * 5
            
            # Adjust turn distance requirement
            if recent_performance > 0.8:
                self.kers_params['min_distance_to_turn'] = max(5, 
                    self.kers_params['min_distance_to_turn'] - self.learning_rate * 2)
            else:
                self.kers_params['min_distance_to_turn'] += self.learning_rate * 2
            
            # Adjust speed range
            if recent_performance > 0.75:
                self.kers_params['min_speed'] = max(20, 
                    self.kers_params['min_speed'] - self.learning_rate * 10)
                self.kers_params['max_speed'] = min(300, 
                    self.kers_params['max_speed'] + self.learning_rate * 10)
            
            # Clear old history
            self.kers_performance_history = self.kers_performance_history[-self.performance_window:]


    def update_path_analysis(self, position, steering_angle):
        """Analyze path to detect straights and upcoming turns"""
        self.path_memory.append((position, steering_angle))
        if len(self.path_memory) > 100:
            self.path_memory.pop(0)
            
        # Calculate distance to next turn
        self.next_turn_distance = self._calculate_distance_to_turn()
        
    def _calculate_distance_to_turn(self):
        """Calculate approximate distance to next significant turn"""
        if len(self.path_memory) < 2:
            return float('inf')
            
        current_pos = self.path_memory[-1][0]
        for i, (pos, angle) in enumerate(reversed(self.path_memory[:-1])):
            if abs(angle) > 0.2:  # Threshold for turn detection
                return i * 1.0  # Approximate distance units
        return float('inf')
        
    def should_deploy_kers(self, current_speed):
        """Determine if KERS should be deployed based on dynamic conditions"""
        # Check if we have enough straight road ahead
        straight_long_enough = self.next_turn_distance > self.kers_params['min_straight_length']
        
        # Check if we're far enough from next turn
        safe_distance = self.next_turn_distance > self.kers_params['min_distance_to_turn']
        
        # Check if speed is in optimal range
        good_speed = (self.kers_params['min_speed'] < current_speed < 
                     self.kers_params['max_speed'])
        
        # Check if we have enough energy
        enough_energy = self.kers_energy > 20
        
        success = all([straight_long_enough, safe_distance, good_speed, enough_energy])
        
        # Update parameters based on success/failure
        self.update_kers_parameters(1.0 if success else 0.0)
    ...
    def full_throttle(self, current_speed, drs_active=False, grip_multiplier=1.0, drag_multiplier=1.0):
        ...
        # Intelligent KERS deployment based on conditions
        if self.should_deploy_kers(current_speed):
            self.kers_active = True
            kers_boost = delta * (self.kers_boost_multiplier - 1)
            used_energy = min(self.kers_energy, kers_boost)
            delta += used_energy
            self.kers_energy -= used_energy
        else:
            self.kers_active = False
    ...
    def heavy_brake(self, current_speed, drs_active=False, grip_multiplier=1.0, drag_multiplier=1.0):
        ...
        # Recover energy through KERS
        recovered = delta * self.kers_recovery_rate
        self.kers_energy = min(self.max_kers_energy, self.kers_energy + recovered)
    ...
    def light_brake(self, current_speed, grip_multiplier=1):
        ...
        # Recover some energy even from light braking
        recovered = delta * self.kers_recovery_rate * 0.5  # Half recovery rate for light braking
        self.kers_energy = min(self.max_kers_energy, self.kers_energy + recovered)
